...
export function PostTitle({ children, favicon }: Props) {
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <h1 className="min-w-0 flex-1 text-left text-balance text-3xl font-bold leading-tight md:text-4xl lg:text-5xl">
        {title ? (
          <>
            {title.slice(0, -2)}
            <span className="whitespace-nowrap">{title.slice(-2)}</span>
          </>
        ) : children}
      </h1>
      {favicon && (
